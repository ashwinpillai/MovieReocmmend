$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = 'ce33be613095974c148aa73cbbc1c3e0';
    var title = $('.movie').val();
    if (title=="") {
  ...
  $.ajax({
    ...
    success: function(movie){
      if(movie.results.length<1){
        ...
        load_movie_details(my_api_key, movie_id, movie_title);
...
function load_movie_details(my_api_key, movie_id, movie_title) {
  // Load basic movie details
  get_movie_videos(movie_id, my_api_key); // Call the function to get movie videos
  
  // Continue with the rest of the code to load other details and display results
  // ...
}
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.results.length > 0) {
        ...
        var videos = response.results.map(function(video) {
          return {
            id: video.id,
            key: video.key,
            name: video.name,
        ...
        // Do something with the videos, such as displaying them or processing them further
        console.log('Videos:', videos);
      } else {
        console.log('No videos found for the movie.');
      }
    },
    error: function() {
      console.error('Failed to retrieve movie videos.');
    }
  });
}
